...
``src/bobvance/conf/local.py``. You can base this file on the
...
.. code-block:: bash
    ...
    $ python src/manage.py test bobvance
...
* ``DB_NAME``: name of the database for the project. Defaults to ``bobvance``.
* ``DB_USER``: username to connect to the database with. Defaults to ``bobvance``.
* ``DB_PASSWORD``: password to use to connect to the database. Defaults to ``bobvance``.
...
2. Start the database and web services:
   ...
   .. code-block:: bash
       ...
       $ docker-compose up -d
       Starting bobvance_db_1 ... done
       Starting bobvance_web_1 ... done
...
3. Create an admin user and load initial data. If different container names
   ...
   .. code-block:: bash
       ...
       $ docker exec -it bobvance_web_1 /app/src/manage.py createsuperuser
       Username: admin
       ...
       Superuser created successfully.

       $ docker exec -it bobvance_web_1 /app/src/manage.py loaddata admin_index groups
...
.. _Github: https://github.com/maykinmedia/bobvance/
...
environment variables. See ``src/bobvance/conf/docker.py`` for
all settings.

.. code-block:: bash

    $ docker build -t bobvance
    $ docker run \
        -p 8000:8000 \
        -e DATABASE_USERNAME=... \
        -e DATABASE_PASSWORD=... \
    ...
    $ docker exec -it bobvance /app/src/manage.py createsuperuser
